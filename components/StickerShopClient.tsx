...
export default function StickerShopClient({ stickers, initialCartOpen = false }: Props) {
  const tickerText =
    "HOT DEAL • สติกเกอร์ใหม่ทุกสัปดาห์ • ซื้อครบรับส่วนลด • อัปเดตแพ็กยอดนิยม • รองรับงานครีเอเตอร์และแบรนด์ • แพ็กน่ารัก แพ็กฮา และแพ็กทำงานครบ • สั่งซื้อไว จ่ายง่าย อัปโหลดสลิปได้ทันที • เช็กสถานะคำสั่งซื้อออนไลน์ตลอด 24 ชั่วโมง •";
  ...
  useEffect(() => {
    ...
    setCheckoutForm(loadInitialCheckoutDraft());
    setHasHydrated(true);
  }, []);
    ...
    fetch("/api/checkout", {
      ...
      .then(async (response) => {
        ...
        if (typeof window !== "undefined") {
          sessionStorage.setItem("line_stick_last_order_id", orderId);
          sessionStorage.setItem("line_stick_last_order_token", paymentToken);
          window.location.replace(`/payment?orderId=${encodeURIComponent(orderId)}&token=${encodeURIComponent(paymentToken)}`);
        }
      })
      .catch((error: unknown) => {
        const message =
  ...
  return (
    <div style={{ background: "#F8F4ED", minHeight: "100vh", fontFamily: "'Noto Sans Thai', 'Poppins', sans-serif" }}>
      ...
      <section
        style={{
          background: "linear-gradient(90deg, #065f2f 0%, #00a43a 50%, #065f2f 100%)",
      ...
      >
        <div className="tickerTrack">
          {[0, 1, 2].map((index) => (
            <span key={index} className="tickerLine">
              {tickerText}
            </span>
          ))}
        </div>
      ...
      <section id="shop-section" className="container py-5">
