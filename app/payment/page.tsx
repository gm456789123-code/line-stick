...
import { promptPayQrDataUrl } from "../../lib/server/promptpay";
...
export default async function PaymentPage({ searchParams }: Props) {
  ...
  const promptPayNumber = String(paymentSettings.promptPayNumber || "").trim();
  const qrCodeDataUrl = promptPayNumber ? await promptPayQrDataUrl(promptPayNumber, total) : "";
      ...
      <div className={styles.container}>
        <header className={styles.hero}>
          <div>
            ...
            <p className={styles.sub}>กรุณาตรวจสอบยอด และอัปโหลดสลิปเพื่อยืนยันการชำระเงิน</p>
          </div>
          ...
          <div className={styles.card}>
            {qrCodeDataUrl ? (
              <div className={styles.qrBox}>
                <h3>PromptPay QR</h3>
                <p>สแกนจ่ายยอดนี้ได้ทันที: {money(total)}</p>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={qrCodeDataUrl} alt="PromptPay QR Code" className={styles.qrImage} />
                <small>เบอร์รับเงิน: {promptPayNumber}</small>
                {paymentSettings.bankName || paymentSettings.bankAccountName || paymentSettings.bankAccountNumber ? (
                  <div className={styles.bankBox}>
                    <p>ธนาคาร: <strong>{paymentSettings.bankName || "-"}</strong></p>
                    <p>ชื่อบัญชี: <strong>{paymentSettings.bankAccountName || "-"}</strong></p>
                    <p>เลขบัญชี: <strong>{paymentSettings.bankAccountNumber || "-"}</strong></p>
                    <p className={styles.bankWarning}>
                      *เลือกสแกน QR code หรือ โอนผ่านธนาคาร และอัปสลิปเพื่อให้แอดมินตรวจสอบ
                    </p>
                  </div>
                ) : null}
              </div>
